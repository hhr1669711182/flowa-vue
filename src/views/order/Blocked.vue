...
const { send: fetchOrderDetail } = useRequest(
  (id: string) => getBlockedOrderDetail(id, authStore.currentCompany ?? undefined),
  { immediate: false }
)

const { send: postReactivate } = useRequest(
  (payload: { id: string; note: string }) =>
    reactivateBlockedOrder({ ...payload, company: authStore.currentCompany ?? undefined }),
  { immediate: false }
)

const { send: postStatus } = useRequest(
  (payload: { id: string; status: BlockedOrderStatus }) =>
    updateBlockedOrderStatus({ ...payload, company: authStore.currentCompany ?? undefined }),
  { immediate: false }
)

const { send: postTicket } = useRequest(
  (payload: { id: string; subject: string; message: string; priority: 'High' | 'Medium' | 'Low' }) =>
    createBlockedSupportTicket({ ...payload, company: authStore.currentCompany ?? undefined }),
  { immediate: false }
)
