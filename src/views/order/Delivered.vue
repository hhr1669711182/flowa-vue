...
const { send: fetchOrderDetail } = useRequest((id: string) => getDeliveredOrderDetail(id), {
  immediate: false
})

const { send: postNote } = useRequest(
  (payload: { id: string; note: string }) => updateDeliveredOrderNote(payload),
  { immediate: false }
)

const { send: postStatus } = useRequest(
  (payload: { id: string; status: DeliveredStatus }) => updateDeliveredOrderStatus(payload),
  { immediate: false }
)

const { send: postTicket } = useRequest(
  (payload: { id: string; subject: string; message: string; priority: 'High' | 'Medium' | 'Low' }) =>
    createDeliveredSupportTicket(payload),
  { immediate: false }
)
