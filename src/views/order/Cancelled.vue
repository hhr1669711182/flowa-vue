...
const { send: fetchOrderDetail } = useRequest((id: string) => getCancelledOrderDetail(id), {
  immediate: false
})

const { send: postReactivate } = useRequest(
  (payload: { id: string; note: string; targetStage: CancelledOrderStage }) => reactivateCancelledOrder(payload),
  { immediate: false }
)

const { send: postStatus } = useRequest(
  (payload: { id: string; status: CancelledOrderStatus }) => updateCancelledOrderStatus(payload),
  { immediate: false }
)

const { send: postTicket } = useRequest(
  (payload: { id: string; subject: string; message: string; priority: 'High' | 'Medium' | 'Low' }) =>
    createCancelledSupportTicket(payload),
  { immediate: false }
)
