...
const tableRows = computed(() => {
  return rawShipments.value.map((r: any) => {
    ...
    const status_text = trackingData?.status || trackingData?.package_status || r?.status_text || 'processing'
    ...
    const display_tracking_no = lastMileTrackingNo || ''
    return {
      ...r,
      tracking,
...
function copyTrackingLink(row: any) {
  copyText(`${location.origin}/#/orders/tracking?kw=${encodeURIComponent(String(row?.sales_order || '').trim())}`)
}
...
async function refresh() {
  ...
  try {
    ...
    if (keys.length) {
      if (!company.value) throw new Error('company is required')
      const res: any = await getTrackingBySalesOrders({ company: company.value, sales_orders: keys })
      const msg = res?.message ?? res
      const merged: any[] = Array.isArray(msg?.data) ? msg.data : []
      const seen = new Set<string>()
      const rows = merged
        .map((x: any) => {
          const shipment = x?.shipment || {}
          const tracking = x?.tracking || null
          const losName = String(shipment?.los_name || shipment?.name || '').trim()
          return {
            _key: losName || `${shipment.sales_order || ''}-${Math.random().toString(36).slice(2, 8)}`,
            sales_order: shipment.sales_order || '',
            destination_country: shipment.destination_country || '',
            primary_tracking_no: '',
            tracking,
            last_mile: x?.last_mile || null,
          }
        })
        .filter((r: any) => {
          const k = String(r._key || '').trim()
          if (!k) return true
      ...
      serverTotal.value = rows.length
      pageIndex.value = 1
      expandedRowKeys.value = rows.length === 1 ? [rowKey(rows[0])] : []
      return
    }

    const res = await getShopShipments({ page: pageIndex.value, page_size: pageSize.value })
    const msg = (res as any)?.message ?? res
    const shipments = msg?.shipments ?? []
    serverTotal.value = msg?.total ?? 0
    const rows = shipments.map((s: any) => ({
      sales_order: s.sales_order || '',
      destination_country: s.destination_country || '',
      waybill_number: s.waybill_number || '',
      tracking_number: s.tracking_number || '',
      primary_tracking_no: s.tracking_number || s.waybill_number || '',
    }))
    rawShipments.value = rows
    const trackKeys = [...new Set(rows.map((r: any) => r.primary_tracking_no).filter(Boolean))]
    if (trackKeys.length) {
      const tr = await batchTracking(trackKeys)
      const data = (tr as any)?.message?.data ?? (tr as any)?.data ?? []
    ...
    ElMessage.error(e?.message || 'Failed to load shipments')
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  getCurrentUserInfo()
    .then((userRes: any) => {
      const msg = userRes?.message ?? userRes
