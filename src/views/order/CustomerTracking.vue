...
async function load() {
  const kw = queryKey.value
  if (!kw) return
  loading.value = true
  try {
    const res = await getPublicTrackingByNumber(kw).send()
    const msg = (res as any)?.message ?? res
    rawRow.value = msg && typeof msg === 'object' ? msg : null
  } catch (e: any) {
    rawRow.value = null
...
.yuntrack-expand-grid { display: grid; grid-template-columns: 1fr 360px; gap: 18px; }
...
.tms-recipient-title { font-weight: 800; color: #0f766e; margin-bottom: 10px; }
.tms-recipient-box { display: flex; flex-direction: column; gap: 8px; }
.tms-recipient-field { display: grid; grid-template-columns: 120px 1fr; gap: 10px; }
.tms-recipient-label { font-size: 12px; color: #6b7280; }
.tms-recipient-value { font-size: 13px; font-weight: 600; word-break: break-word; }
