...
const companyOptions = computed(() =>
  authStore.companies?.length ? authStore.companies : authStore.company ? [authStore.company] : []
)
const form = reactive({
  amount: 5000,
  company: ''
...
async function onSubmit() {
  ...
  const company = form.company || authStore.company || companyOptions.value[0] || undefined
  if (!company) {
    ElMessage.warning('Please select a company or wait for company to load')
    return
  }
  loading.value = true
  // 先同步打开空白窗口，避免 await 之后被浏览器拦截弹窗。
  const payWindow = window.open('', '_blank')
  try {
    const res = await createRechargePayment({ amount, company })
    // Frappe 常见结构：{ message: { success, data: { payment_url } } }，统一解包。
