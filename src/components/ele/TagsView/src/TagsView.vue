...
import { ContextMenu, ContextMenuExpose } from '@/components/ele/ContextMenu'
...
const toLastView = () => {
  ...
  if (latestView) {
    ...
    if (
      permissionStore.getAddRouters[0] &&
    ...
    if (permissionStore.getAddRouters[0]) {
...
const moveToTarget = (currentTag: RouteLocationNormalizedLoaded) => {
  ...
  if (tagList.length > 0) {
    if (tagList.length > 0) {
      firstTag = tagList[0] as any
      lastTag = tagList[tagList.length - 1] as any
    }
  }
