...
import { Menu } from '@/components/ele/Menu'
...
export default defineComponent({
  ...
  setup() {
    ...
    const tabClick = (item: AppRouteRecordRaw) => {
      ...
      tabActive.value = item.children ? item.path : (item.path as string).split('/')[0]
      if (item.children) {
    ...
    const isActive = (currentPath: string) => {
      ...
      if (tabPathMap[currentPath] && tabPathMap[currentPath].includes(path)) {
        return true
      }
      return false
    }
