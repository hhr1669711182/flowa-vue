...
import type { BlockedOrderRecord, BlockedOrderStatus } from '@/api/orderBlocked'
...
const seedBlockedOrders = (): BlockedOrderRecord[] => {
  ...
  return Array.from({ length: 82 }).map((_, index) => {
    ...
    return {
      ...
      customerName: ['Ava Cooper', 'Ethan Ward', 'Luna Price', 'Lucas Ross', 'Ella James'][index % 5]!,
      customerRegion: ['UK/London', 'US/New York', 'DE/Munich', 'FR/Paris', 'CA/Toronto'][index % 5]!,
      inventoryStatus: inventoryPool[index % inventoryPool.length]!,
      createDate: formatDate(created),
      blockedDate: formatDate(blocked),
    ...
    blockedOrdersDb = blockedOrdersDb.map((item) =>
      item.id === data.id
        ? {
            ...
            status: 'Resolved',
            holdLevel: 'Low',
            holdNote: data.note || 'Reactivated by operator.'
          }
        : item
    )
    return { success: true }
