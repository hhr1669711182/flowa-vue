import { alovaInstance, ensureFrappeCsrfToken, getFrappeCsrfToken } from '@/services/alova'
import { OMS_API, withApiBase } from '@/api/omsApiBase'
...
export interface ChatMessage {
  /** Trouble Ticket Reply name, e.g. OMSTTR-00001 */
  id: string
  sender: 'user' | 'support'
  senderName: string
  avatar?: string
  ...
  image?: string
  fileUrl?: string
  /** Raw sent_at from server for date chips */
  sentAtRaw?: string
}
...
export interface Ticket {
  ...
  stage?: string
  stageDetail?: string
  /** 关联单据类型 Sales Order / Material Request */
  ...
  /** DocField `created_by` (User) */
  createdBy?: string
  /** DocField `assigned_to` / 指派给 (User) */
  assignedTo?: string
...
export function ticketAssetUrl(path: string | undefined | null): string {
  ...
  return withApiBase(p.startsWith('/') ? p : `/${p}`)
}

/** 工单 image / file_attachment 字段可能为逗号或换行分隔的多路径 */
...
export async function getTicketReplies(ticketName: string): Promise<ChatMessage[]> {
  const auth = useAuthStore()
  const company = (await auth.ensureCompany()) || undefined
  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_list_trouble_ticket_replies`, {
...
export async function appendTicketReply(ticketName: string, content: string) {
  ...
  const auth = useAuthStore()
  const company = (await auth.ensureCompany()) || undefined
  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_append_trouble_ticket_reply`, {
    trouble_ticket: ticketName,
    company,
    content: text,
    content_en: text,
...
export const getTickets = async (query: TicketsQuery & { company?: string } = {}): Promise<TicketsResponse> => {
  const auth = useAuthStore()
  const company = (query.company?.trim() ? query.company.trim() : await auth.ensureCompany()) || undefined
  const page = query.page ?? 1
  const pageSize = query.pageSize ?? 20

  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_list_trouble_tickets`, {
    company,
    search: query.search || '',
    status: query.status || '',
...
export const getTicketDetail = async (name: string): Promise<Ticket | null> => {
  const auth = useAuthStore()
  const company = (await auth.ensureCompany()) || undefined
  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_get_trouble_ticket`, { name, company })
  const msg = unwrapFrappeMessage(raw) as Record<string, unknown> | undefined
...
export type CreateTicketPayload = {
  ...
  priority?: TicketPriority
  /** 一般由 `suggestTroubleTicketAssignee` 在 Category onChange 时写入；与后端值班规则一致 */
  assigned_to?: string
...
/** 创建工单时检索关联 Sales Order / Material Request（公司范围内，接近 Desk Link 搜索）。 */
export async function searchTroubleTicketReferences(p: {
  reference_doctype: 'Sales Order' | 'Material Request'
  txt?: string
  limit?: number
}): Promise<TroubleTicketRefSuggestion[]> {
  const auth = useAuthStore()
  const company = (await auth.ensureCompany()) || undefined
  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_search_trouble_ticket_reference`, {
    company,
...
/** 新建工单：按问题大类解析 Trouble Duty Rule 得到指派 User；无规则时为 Administrator。供 Category onChange 调用。 */
export async function suggestTroubleTicketAssignee(p: {
  company?: string
  problem_category: string
}): Promise<string> {
  const auth = useAuthStore()
  const company = (p.company?.trim() || (await auth.ensureCompany()) || '').trim()
  if (!company) {
    throw new Error('Company is required')
  }
  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_suggest_trouble_ticket_assignee`, {
    company,
...
export const createTicket = async (payload: CreateTicketPayload) => {
  const auth = useAuthStore()
  const company = (payload.company?.trim() || (await auth.ensureCompany()) || '').trim()
  if (!company) {
    throw new Error('Company is required')
  }
  ...
  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_create_trouble_ticket`, {
    company,
    subject: subj,
    subject_en: subjEn,
    description: desc,
...
export async function uploadTicketAttachment(file: File): Promise<string> {
  ...
  const url = withApiBase('/api/method/upload_file')
  const res = await fetch(url, { method: 'POST', body: fd, credentials: 'include', headers })
  ...
  if (!res.ok) {
    const err =
      json && typeof json === 'object' && (json as any)._server_messages
        ? String((json as any).exc || 'Upload failed')
        : 'Upload failed'
    throw new Error(err)
  }
  const message = json.message as Record<string, unknown> | string | undefined
...
export const updateTicketStatus = async (name: string, status: TicketStatus) => {
  const auth = useAuthStore()
  const company = (await auth.ensureCompany()) || undefined
  const raw = await alovaInstance.Post<unknown>(`${OMS_API}.flowa_update_trouble_ticket_status`, {
    name,
    status,
    company,
  })
...
/** @deprecated 使用 appendTicketReply */
